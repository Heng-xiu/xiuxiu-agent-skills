...
- `shot-annotate`: Turn authenticated web dashboards (the pages AI normally can't see behind login walls) into textbook-grade annotated visual guides — drive the user's own logged-in browser to capture real screenshots, verify every shot, overlay numbered annotation pins with plain-language legends, run a PII audit before publishing, and honestly flag discrepancies against prior assumptions as errata. macOS-first (AppleScript + screencapture), with a Playwright headed-login fallback. Landing page: [heng-xiu.github.io/xiuxiu-agent-skills/shot-annotate](https://heng-xiu.github.io/xiuxiu-agent-skills/shot-annotate/).
